...
private fun SmsForwardAddressSetting(m: ChatModel) {
  ...
  LaunchedEffect(smsForwardAddress.value) {
    val currentAddress = smsForwardAddress.value
    val wasAddress = lastSmsForwardAddress
    val isDefaultSmsApp = if (appPlatform.isAndroid) platform.androidIsDefaultSmsApp() else null
    Log.d(
      SMS_FORWARD_LOG_TAG,
      "Observed SMS forward address change: current=$currentAddress, last=$wasAddress, isDefaultSmsApp=$isDefaultSmsApp"
    )

    val addressChanged = currentAddress != wasAddress
    val changedToValid = addressChanged && currentAddress != null

    when {
      !addressChanged -> Log.d(SMS_FORWARD_LOG_TAG, "Address unchanged; skipping SMS app chooser request")
      !appPlatform.isAndroid -> Log.d(SMS_FORWARD_LOG_TAG, "Address changed but platform is not Android; no SMS app chooser available")
      !changedToValid -> Log.d(SMS_FORWARD_LOG_TAG, "Address changed to DoNotForward; chooser only required when enabling forwarding")
      isDefaultSmsApp == false -> {
        Log.d(SMS_FORWARD_LOG_TAG, "App not default; address set to valid target; requesting SMS app chooser")
        platform.androidShowDefaultSmsAppChooser(forceWhenAlreadyDefault = false)
      }
      isDefaultSmsApp == true -> Log.d(SMS_FORWARD_LOG_TAG, "App already default; valid address selected; no chooser needed")
      else -> Log.d(SMS_FORWARD_LOG_TAG, "Default SMS app state unknown; skipping chooser to avoid unintended prompt")
    }
    lastSmsForwardAddress = currentAddress
  }
  ...
  ExposedDropDownSettingRow(
    ...
    onSelected = { address ->
      val currentAddress = smsForwardAddress.value
      Log.d(SMS_FORWARD_LOG_TAG, "Address selection: chosen=$address, current=$currentAddress")
      if (address != currentAddress) {
        Log.d(SMS_FORWARD_LOG_TAG, "Persisting new SMS forward address selection")
        appPrefs.smsForwardAddress.set(address)
      } else {
        Log.d(SMS_FORWARD_LOG_TAG, "Selection matches current address; no preference update")
      }
    }
  )
